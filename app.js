...
function scrollEst(){
  try{
    const c=document.getElementById('estimateur');
    const y=c.getBoundingClientRect().top+window.pageYOffset-100; // -100 : passe sous le header du site
...
function next(){
  ...
  if(state.step<STEPS){setStep(state.step+1);scrollEst();}
}
function back(){ if(state.step>1){setStep(state.step-1);scrollEst();} }
